...
async def init_db():
    ...
    if db_path.exists():
        try:
            ...
            if 'hemis_login' not in columns:
                cursor.execute("ALTER TABLE users ADD COLUMN hemis_login VARCHAR(100) NULL")
                conn.commit()
                print("✅ Added hemis_login column to users table")
                
            if 'hemis_password' not in columns:
                cursor.execute("ALTER TABLE users ADD COLUMN hemis_password VARCHAR(100) NULL")
                conn.commit()
                print("✅ Added hemis_password column to users table")
                
            if 'group_id' not in columns:
                cursor.execute("ALTER TABLE users ADD COLUMN group_id INT REFERENCES groups (id) ON DELETE SET NULL")
                conn.commit()
